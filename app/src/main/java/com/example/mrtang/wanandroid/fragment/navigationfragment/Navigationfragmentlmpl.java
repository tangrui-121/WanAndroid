...
public class Navigationfragmentlmpl implements CallBackListener, INavigationPresenter {
    private INavigationView iView;
    private Request request;

    public Navigationfragmentlmpl(INavigationView iView) {
        this.iView = iView;
    }

    public void getNavigationData(Context context) {
        request = RequestUtils.getInfo(Consts.SERVER_IP_navigation);
        if (context == null) {
            ...
            case 1:
                if (success) {
                    if (data.get().getIntValue("errorCode") == Result.RESULT_OK) {
                        List<NavigationGroupBean> items = NavigationGroupBean.parseFromJson(data.get().getJSONArray("data"));
                        if (items != null && items.size() != 0) {
                            iView.initData(items);
                        } else {
                            iView.showError("暂时木有导航数据~~");
                        }
                    } else {
